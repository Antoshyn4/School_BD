...
namespace KT_2
{
    public partial class AddSt : Form
    {
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            if (textBox3.Text != "")
            {
                DataRow[] dr = for_kursDataSet.Olympiads.Select("[Student code] =" + textBox3.Text);
                if (dr.Count() > 0)
                {
                    foreach (DataRow item in for_kursDataSet.Olympiads.Rows)
                    {
                        if (item["Student code"].ToString() == textBox3.Text)
                        {
                            item.Delete();
                        }
                    }
                ...
                DataRow[] dr1 = for_kursDataSet.Duty.Select("[Student code] =" + textBox3.Text);
                if (dr1.Count() > 0)
                {
                ...
                DataRow[] dr2 = for_kursDataSet.Magazine.Select("[Student code] =" + textBox3.Text);
                if (dr2.Count() > 0)
                {
                ...
                DataRow[] dr3 = for_kursDataSet.Students.Select("[Student code] =" + textBox3.Text);
                if (dr3.Count() > 0)
                {
